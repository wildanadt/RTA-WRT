...
# Constants
readonly FIRMWARE_DIR="$GITHUB_WORKSPACE/$WORKING_DIR/compiled_images"
readonly RELEASE_URL="https://github.com/rizkikotet-dev/RTA-WRT/releases/download/${RELEASE_TAG}"

rename_firmware() {
    echo -e "${STEPS} Starting firmware file renaming process..."
    ...
    local search_replace_patterns=($(define_patterns))
    ...
    echo -e "${INFO} Processing files in ${FIRMWARE_DIR}..."
    local renamed_count=$(process_files)

    # Final status
    sync && sleep 1
    echo -e "${INFO} Current directory contents:"
    ls -lh
    echo -e "${SUCCESS} Renamed ${renamed_count} files successfully."
}
